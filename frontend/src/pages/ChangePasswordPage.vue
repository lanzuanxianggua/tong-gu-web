<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <h1 class="auth-card__title">{{ t('auth.changePassword') }}</h1>
      <div class="auth-card__deco-line"></div>

      <el-form :model="formData" :rules="rules" ref="changeFormRef" class="auth-form" @keyup.enter="handleSubmit">
        <el-form-item prop="old_password">
          <div class="auth-field">
            <label class="auth-field__label">{{ t('auth.oldPassword') }}</label>
            <input v-model="formData.old_password" :type="showOld ? 'text' : 'password'" class="auth-field__input" :placeholder="t('auth.placeholders.password')" />
            <button type="button" class="auth-field__toggle" @click="showOld = !showOld">
              {{ showOld ? t('auth.hide') : t('auth.show') }}
            </button>
          </div>
        </el-form-item>
        ...
        <el-form-item prop="new_password">
          <div class="auth-field">
            ...
            <input v-model="formData.new_password" :type="showNew ? 'text' : 'password'" class="auth-field__input" :placeholder="t('auth.placeholders.newPassword')" />
            <button type="button" class="auth-field__toggle" @click="showNew = !showNew">
        ...
        <el-form-item prop="confirm_password">
          <div class="auth-field">
            ...
            <input v-model="formData.confirm_password" :type="showNew ? 'text' : 'password'" class="auth-field__input" :placeholder="t('auth.placeholders.confirmPassword')" />
          </div>
        </el-form-item>

        <div v-if="formData.new_password" class="password-hint">
      ...
      <div class="auth-footer">
        <router-link to="/profile" class="auth-footer__link">← {{ t('auth.backToProfile') }}</router-link>
      </div>
...
const rules = {
  old_password: [{ required: true, message: () => t('auth.oldPassword'), trigger: 'blur' }],
  new_password: [{ required: true, message: () => t('auth.newPassword'), trigger: 'blur' }],
  confirm_password: [{ required: true, message: () => t('auth.confirmPassword'), trigger: 'blur' }],
}
...
async function handleSubmit() {
  ...
  try {
    ...
    // error handled by interceptor for network errors
